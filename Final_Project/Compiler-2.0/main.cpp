...
// Split a string like "E + T" into words
...
void eliminateLeftRecursion(vector<string> &nonTerminals, map<string, vector<string>> &productions)
{
    ...
    {
        string A = nonTerminals[i];
        vector<string> recursivePart;    
        vector<string> nonRecursivePart; 
        ...
        for (int j = 0; j < (int)nonRecursivePart.size(); j++)
        {
            ...
            if (beta == EPSILON)
                newA.push_back(Aprime);
            else
                newA.push_back(beta + " " + Aprime);
        }
        for (int j = 0; j < (int)recursivePart.size(); j++)
        {
            string alpha = recursivePart[j];
            if (alpha == "")
                newAprime.push_back(Aprime); 
            else
                newAprime.push_back(alpha + " " + Aprime);
        }
                     ...
                     vector<string> &outSymbols)
{
    ...
    for (int i = start; i < (int)words.size(); i++)
    {
        ...
        {
            if (firstOfSym[k] == EPSILON)
                hasEpsilon = true;
            else
                addIfNew(outSymbols, firstOfSym[k]);
        }
        if (!hasEpsilon)
        {
            nullable = false;
...
map<string, vector<string>> computeFirstSets(vector<string> &nonTerminals, map<string, vector<string>> &productions)
{
    map<string, vector<string>> FIRST;
    for (int i = 0; i < (int)nonTerminals.size(); i++)
        FIRST[nonTerminals[i]] = vector<string>();
    ...
    while (changed)
    {
        ...
        for (int i = 0; i < (int)nonTerminals.size(); i++)
        {
            ...
            for (int j = 0; j < (int)rhsList.size(); j++)
            {
                ...
                {
                    if (!contains(FIRST[A], EPSILON))
                    {
                        FIRST[A].push_back(EPSILON);
                        changed = true;
                    }
                    continue;
                }
                ...
                for (int k = 0; k < (int)found.size(); k++)
                    addIfNew(FIRST[A], found[k]);
                if (nullable)
                    addIfNew(FIRST[A], EPSILON);
                if ((int)FIRST[A].size() != before)
                    changed = true;
            }
        }
    }
                                              ...
                                              map<string, vector<string>> &FIRST, string startSymbol)
{
    ...
    for (int i = 0; i < (int)nonTerminals.size(); i++)
        FOLLOW[nonTerminals[i]] = vector<string>();
        ...
        for (int i = 0; i < (int)nonTerminals.size(); i++)
        {
            ...
            for (int j = 0; j < (int)rhsList.size(); j++)
            {
                ...
                for (int pos = 0; pos < (int)words.size(); pos++)
                {
                    ...
                    for (int k = 0; k < (int)found.size(); k++)
                        addIfNew(FOLLOW[B], found[k]);
                    if (nullable)
                    {
                        for (int k = 0; k < (int)FOLLOW[A].size(); k++)
                            addIfNew(FOLLOW[B], FOLLOW[A][k]);
                    }
                        ...
                        string startSymbol, vector<string> inputTokens)
{
    ...
    while (!stack.empty())
    {
        ...
        {
            cout.width(16);
            cout << stackToString(stack);
            cout.width(20);
            cout << remainingInputToString(input, pos);
            cout << "Accepted\n";
            return true;
        }
    ...
    while (foundOne)
    {
        ...
        for (int i = 0; i < (int)list.size(); i++)
        {
            if (list[i] == opSymbol)
            {
                ...
                for (int k = 0; k < i - 1; k++)
                    updated.push_back(list[k]);
                updated.push_back(newTemp);
                for (int k = i + 2; k < (int)list.size(); k++)
                    updated.push_back(list[k]);
                list = updated;
...
int main(int argc, char **argv)
{
    ...
    vector<string> exprTerminals;
    vector<string> exprAtoms;    
    ...
    else
    {
        ...
        for (int i = 0; i < (int)asmLines.size(); i++)
            cout << asmLines[i] << "\n";
    }
